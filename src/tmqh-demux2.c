 ...
 *
 * 2-way demultiplexing queue handler
 */
...
void TmqhInputDemux2ShutdownHandler(ThreadVars *tv) {
    int i;
 ...
 */
SCDQGenericQData *TmqhInputDemux2OnQ(SCDQDataQueue *q)
{
    SCMutexLock(&q->mutex_q);
    if (q->len == 0) {
        /* if we have no packets in queue, wait... */
 ...
 */
void TmqhOutputDemux2OnQ(SCDQDataQueue *q, SCDQGenericQData *data)
{
    SCMutexLock(&q->mutex_q);
